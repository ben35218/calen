> **ENGINEERING COMPANION — not imported into the admin portal.**
> The tester-facing plan (the one imported into Quality → Test cases and run
> before each release) is [PRE-RELEASE-TEST-PLAN.md](PRE-RELEASE-TEST-PLAN.md).
> This file preserves the original full-depth technical plan: the app-behavior
> sections are superseded by the tester plan, but the engineering-only material
> — §0 environments/accounts, §24 security & privacy audits, §25 App Store
> review readiness, §26 upgrade/migration, §27 release ops & exit criteria,
> and Appendix A's regression list — is still run by the engineer, from here.
