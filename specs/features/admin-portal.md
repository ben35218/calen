...
| Monetization | `GET/PUT /api/monetization-config` | Dirty-state tracking, client+server validation, review-diff confirm, unsaved-changes leave guards. Edits the **chat token-pricing knobs** (`credits.appleFeePct`, `credits.chatMargin` — validated [0,1) / [1.0,1.5]; chat is token-priced, not a flat debit), the flat per-action credit prices (`credits.actionCosts` — the debited knob, with `chat` **excluded** from the price grid since it's token-priced), the raw provider token rates — **per token type** (`tokenRatesPer1M`: a 4-field row per model family for input/output/cacheRead/cacheWrite $/1M; legacy numeric entries are expanded to the per-type shape on load and bare numbers are rejected on save; these price the chat debit AND feed reconciliation), and the Calen AI plan card (`aiPlan`: productId/price/monthlyCredits); the view backfills `actionCosts`/`appleFeePct`/`chatMargin`/`aiPlan` defaults on docs predating them. Behavior of the values: [billing-plans](billing-plans.md). |
...
| Test cases | `GET/POST /api/admin/qa/cases`, `GET …/cases/:id/history`, `PUT …/cases/:id`, `POST …/cases/import` | The library releases are tested against, imported from the repo's own plan document (markdown or CSV, read client-side and posted as text — no multipart). Import is **dry-run first**, with the four-bucket diff (added/updated/unchanged/**retired**) shown for confirm — the same review-diff rule as the monetization and email configs. **Tapping a case opens its detail** — full steps/expected, cross-run history, and a record-result control against any in-progress run (same upsert as the run screen). A repo-owned case is not editable here; a case dropped from the document is retired, never deleted. Behavior: [release-qa](release-qa.md). |
