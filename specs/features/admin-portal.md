...
| Monetization | `GET/PUT /api/monetization-config` | Dirty-state tracking, client+server validation, review-diff confirm, unsaved-changes leave guards. Edits the flat per-action credit prices (`credits.actionCosts` — the debited knob), the raw provider-cost reference rates (reconciliation only), and the Calen AI plan card (`aiPlan`: productId/price/monthlyCredits); the view backfills `actionCosts`/`aiPlan` defaults on docs predating them. Behavior of the values: [billing-plans](billing-plans.md). |
