...
- **An event alert counts back from the event's ALERT ANCHOR, which is not always
  ...
  - Because an all-day event's anchor is a wall-clock hour on a date, its offsets
    ...
    those (plus None and Custom…, whose sheet is **fixed to the Days unit**), and
    so does the event detail view — its alert rows are live pickers reading the
    same shared builder, so an all-day event can never be offered minute offsets
    on one surface and days on the other (see
    [calendar.md](calendar.md) → Reminders/alerts). Both label them with the hour
    they fire at ("1 day before (9:00 AM)"), and the AI form-assist /
    `open_event_form` schemas advertise only whole-day values while the event is
    all-day.
...
- **When the window is recomputed.** `rescheduleReminders()` runs on three
  ...
  2. **Any calendar or trip data change.** The hook subscribes to the React
     Query cache and reschedules ~1.5s after a `['calendar']` **or `['trips']`**
     invalidation (debounced, so one save's burst of invalidations is one
     pass). Every calendar mutation in the app already invalidates the first
     key and every booking save (the form's or the booking view's live alert
     pickers) the second, so forms, detail screens, the assistant, templates,
     invitations, and the calendar prefs are all covered without each call site
     remembering to schedule. **This trigger is normative**: without it, an
     alert set on an event never reached the OS until the next
...
- **Tap routing:** a notification tap navigates by the payload's `data.type` —
  ...
  that event's detail; a guardian-recovery approval
  (`guardian_recovery_approved`) opens the Guardian recovery **recover**
  screen, which resumes the persisted request into PIN entry. Cold-start taps ride
...
- Push is best-effort — an invited user who denied notifications, missed the
  ...
  (`householdApi.joinRequests`, always actionable), guardian recovery
  requests awaiting the user's approval (`keysApi.guardianRequests`), and the
  user's **own** in-flight guardian recovery once it has been approved
  (`getRecoveryProgress` — the keychain slot + one poll; no server list
  exists, and without a stored slot the lane is a no-op keychain read). Gated
  to the full app shell — the viewer/paywall shells have no Invitations inbox
  to route into.
...
- **A guardian-recovery approval presents apart too, and first** — the
  requester's own recovery, once their guardian approves, is the highest-value
  alert the lane can show (the user is locked out and one PIN away from their
  data). One fresh approval (there is at most one — opening a request cancels
  any prior): title "Recovery Approved", "«guardian» approved your recovery
  request. Enter your recovery PIN to unlock your data on this device.",
  **Enter PIN / Not Now**, routed at the Guardian recovery **recover** screen.
  Prompted once per `requestId`; skipped (still marked prompted) when the
  recover screen is already the current route — the user is looking at the PIN
  field the alert would point at. The approval push carries
  `data.type: 'guardian_recovery_approved'` so a foreground arrival triggers
  the same pop-up. Feature behavior owned by
  [guardian-recovery.md](guardian-recovery.md).
...
- **At most TWO showings, ever:** the promo once, then a single re-nudge no
  ...
  Profile **widget promo card** (iOS only, below the menu groups; normative
  in [calendar.md](calendar.md) "Home-screen widget"). A show count without a
