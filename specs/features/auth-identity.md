...
- **Passkey:** a Face ID assertion signs in, and (when it can) unlocks E2EE in
  ...
  - **Usernameless (default, no email typed):** `POST /auth/passkey/challenge`
    ...
    — so the device supplies its own: `lib/passkeyHints` caches every passkey
    credential's `{credentialId, prfSalt}` whenever the factor list passes
    through an unlock (all of `unlockWithPasskey`/`unlockWithPasskeyPrfOutput`/
    `ensureEnrolledOnLogin`/`unlockWithPassword`/`unlockWithRecoveryCode`, plus
    `addPasskeyFactor` seeding the new credential), and the last-used entry's
    salt rides the sign-in assertion as a **top-level `prf.eval`** (WebAuthn
    forbids `evalByCredential` next to an empty `allowCredentials`). When the
    picker chooses the hinted credential — signing back into this device's
    account, the overwhelmingly common case — the one Face ID gesture signs in
    AND unlocks E2EE, same as username-first. A foreign pick (its PRF output
    was evaluated at the wrong salt, so it's discarded), a first-ever sign-in
    on the device (no hint yet), or a stale hint whose factor won't open falls
    back to the post-auth path a relaunch uses (biometric device-key cache,
    then a second passkey assertion) — and that unlock refreshes the hint, so
    the next sign-in is one prompt again. The hint cache DELIBERATELY survives
    sign-out (logout wipes the biometric device-key cache, so before the hint
    existed every sign-in after a sign-out double-prompted): both halves are
    public metadata — the server hands the salt to anyone who posts the
    account's email to `/challenge` — and neither yields anything without the
    passkey itself.
...
### Session persistence & restore (when a device asks to sign in again)

The design goal: a device that has signed in once asks for credentials again
only when the server has actually ended the session. Backgrounding, bad
connectivity, a mid-deploy 5xx, or a typo'd current password must never cost a
sign-in — each of those used to, which is why testers saw daily forced logins.

- **Token lifetime is 7 days, sliding.** `signToken` (middleware/auth) mints
  JWTs with `JWT_EXPIRES_IN` (7d in prod); past the half-life any
  authenticated response carries `X-Refreshed-Token` and the client swaps its
  stored copy — an install used at least weekly never re-authenticates.
- **Cold-start restore is failure-split** (store/auth bootstrap): `loadToken`
  → `GET /auth/me` (15s timeout — the only call that sets one; AI requests
  legitimately run long).
  - **401/403** — the server saw the token and rejected it: token + cached
    profile cleared, sign-in required. This is the ONLY path that deletes a
    stored token.
  - **Unreachable** (offline / timeout / 5xx) — the token is KEPT; the app
    enters on the cached last `/auth/me` payload (`lib/authCache` —
    plaintext profile in AsyncStorage, same sensitivity class as the
    calendar-prefs cache, wiped by sign-out) plus the replica, then
    re-verifies in the background (once after ~20s, and on each return to
    foreground) until a fetch lands. With no cached profile (a first sign-in
    whose `/auth/me` never landed) the login screen shows, but the token
    survives for the next launch.
- **The global 401 sign-out is scoped** (`shouldSignOutOn401`, api/client): a
  401 runs the sign-out teardown only when the failing request actually
  CARRIED the bearer token (a request racing the cold-start token load, or a
  pre-auth endpoint, proves nothing about the stored session) and is not one
  of the credential-check endpoints (`PUT /auth/email`, `PUT /auth/password`,
  `DELETE /auth/account`) — kept excluded client-side for the transition
  window where a new app meets an old server. The server now answers a wrong
  current password there with **403**: 401 is reserved for "this session is
  dead" (missing/expired token, revoked session row). Convention pinned in
  [api-reference](../platform/api-reference.md) "Conventions".
- **Two sign-out reasons, one teardown** (`signOut(reason)` in store/auth; the
  full wipe list is the next section). A session-EXPIRY sign-out (the 401
  handler) differs from a deliberate one in exactly two ways:
  - it **keeps the device-key cache** (both tiers — silent and biometric, see
    crypto-e2ee.md), so the re-sign-in that follows a weekly expiry is at most
    a Face ID glance + one-tap passkey instead of a typed password. Safe
    because the cache blob is **owner-stamped**:
    `cacheKeyPairToDevice` writes the account id into the blob, and
    `unlockFromDeviceCache` refuses AND drops a blob stamped for a different
    account (a legacy unstamped blob is accepted once and upgraded in place),
    so a different account signing in later can never inherit the previous
    account's identity key;
  - it **skips the push-token unregister** — the dead token can't authorize
    it anyway (before the split, that doomed call's own 401 re-fired the
    handler; a re-entrancy latch in `signOut` also guards the teardown).
...
(the deliberate, user-intent sign-out — the session-expiry variant above keeps
the device key and skips the push unregister) drops, in order: the in-memory
E2EE key + biometric device-key cache, the
...
notifications.md), the token, the cached `/auth/me` profile (`lib/authCache`),
...
calendars exist, who they're shared with, their colors, order and visibility.
...
mirror-image reason. Privacy — it leaks calendar names, colors and outside-share
...
- **App lock** (A4, `useAppLock`): the app can require Face ID again after being
  ...
  security → Data & privacy controls. **The pref now governs cold starts too**:
  at "Never" (the default) the device keeps a silent, owner-stamped copy of the
  identity key (crypto-e2ee.md "The device key cache: two tiers"), so relaunch
  after an overnight process eviction restores keys with **zero prompts** —
  before this, every cold start demanded a biometric or passkey no matter what
  the pref said, which read as the setting being broken. Choosing any lock
  window deletes the silent copy immediately (`applyAppLockPolicy`, wired to
  the pref chips), so from the next cold start on, re-entry is Face ID — the
  prompt returns as an explicit user choice. The pref stays device-local.
...
- Account self-service: `GET /auth/me`, `PUT /auth/email`, `PUT /auth/password`,
  `DELETE /auth/account` (immediate full deletion). A wrong
  `currentPassword`/`password` on these answers **403** (valid session, failed
  credential input) — never 401, which the client reads as session death (see
  "Session persistence & restore"). Deletion is
