...
- **Two sign-out reasons, one teardown** (`signOut(reason)` in store/auth; the
  ...
  - it **keeps the biometric device-key cache**, so the re-sign-in that
    follows a weekly expiry is a Face ID glance + one-tap passkey instead of a
    typed password. Safe because the cache blob is now **owner-stamped**:
