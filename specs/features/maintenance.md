...
- **A date the repeat rule never generates can't be saved as the series'
  ...
  `lib/recurrence.ts`, unit-tested) covers weekly weekday sets, monthly
  numbered dates, monthly/yearly ordinal weekdays (including *which* one of
  the month — second/last/next-to-last — and the weekday/weekend/day kinds by
  class), and yearly months; daily and plain-interval rules pin no days and
  never conflict. **The "Save for This Chore/Task Only" scope is exempt**: it
  detaches the occurrence as a one-time copy, which repeats nothing and may
  land on any day — that flow ("do this one on Friday instead") must keep
  working. The whole-series and Save-for-Future scopes keep the rule, so
  picking either with a mismatched date shows the error instead of saving.
