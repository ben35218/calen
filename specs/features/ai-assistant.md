...
- Follow-up chips come from the `suggest_followups` tool inside the same
  ...
  photo/text" flows. On device those flows all render through the ONE shared
  `FormAssist` card (mobile `components/FormAssist.tsx`) at the top of an
  add/edit form: the `CalenGlyph` gradient-"C" brand mark + "Ask Calen"
  header — the same mark the calendar's assistant FAB wears, so Calen looks
  the same at every entry point (it is never tinted, so where the assistant
  sits on an accent-filled disc the white-on-color `CalenChatIcon` stands in)
  — **collapsed by default**
  (tap to expand + focus the prompt; a flow that lands with the assistant as
  the expected next step — the recipe form after a quick import — mounts it
  expanded without stealing the keyboard), the action button tinted with the
  section accent in accented areas. The recipe form reuses the same card with
  a custom submit (`/edit-with-ai`, "Apply changes") instead of the
  `/form-assist` fill.
