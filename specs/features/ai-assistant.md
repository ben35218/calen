...
tests:
  - server/src/routes/formAssist.multiTurn.test.js
  - mobile/src/components/__tests__/FormAssistChat.test.tsx
  - mobile/src/lib/__tests__/formAssistTranscript.test.ts
  ...
  - mobile/src/screens/chat/__tests__/AssistantScreen.gate.test.tsx
---
...
- Follow-up chips come from the `suggest_followups` tool inside the same
  ...
  reply). `POST /form-assist` powers "fill this form from a plain-language
  description" on every add/edit form — see **Ask Calen on a form** below.
...
- **Navigation & setup shortcuts** (`services/navDestinations.js` ↔
  ...
  - **`setup`** (gear chip) — **guided configuration for an actionable gap.**
    ...
    **separate** `CONFIG_DESTINATIONS` map (an `all` group applied to every
    ...
    | `setup_contact` | Add this contact | calendar | user wants to call/email a professional not saved / with no number | Contact form phone section (`ContactForm { type:'service', focus:'phone' }`) |
...
- **Chat history is device-local, 7 days** (`lib/chatHistory.ts`): the four
  ...
  nothing persists.
- **The unified Calen view is add-on aware** (`AssistantScreen`): the four-tab
  switcher (Calendar / Chores / Maintenance / Trips — `ASSISTANT_TABS`) always
  renders every tab regardless of entitlements — hiding a tab would bury the
  add-on it sells and reflow the row per household — but a tab whose add-on
  isn't owned (`addon` on the tab entry: `chores` / `maintenance` / `trips`;
  Calendar is never gated) swaps its chat body for the same `AddonLockedView`
  interstitial the feature home screens render, with the switcher row kept
  above it so the user can hop straight back to an unlocked assistant. This
  closes the one entry path that skipped the add-on gate (the Calen view is
  reachable from the always-available calendar FAB) and keeps the assistant
  from drafting records into a locked feature. Gating reads `useOwnedAddons`
  (locked-by-default until the cached entitlement set loads — `CenteredLoader`
  in the area accent per the loading rule, matching ChoresScreen), and a
  cross-tab resume hand-off targeting a locked tab hits the same gate at
  render. The standalone `TripAssistant` route needs no gate of its own — it is
  only reachable from inside the trips area, behind TripsScreen's locked view.
...
### Ask Calen on a form (normative)

Every add/edit form reaches Calen the same way: a **floating "Ask Calen" pill**
bottom-right (`components/FormAssistChat.tsx`, rendering
`<AssistantButton label>` through `<Screen floating>`), opening a **half-screen
chat sheet**. The pill carries the `CalenGlyph` gradient-"C" brand mark on the
left — the same mark the calendar's assistant FAB wears, so Calen looks the same
at every entry point (it is never tinted, so where the assistant sits on an
accent-filled disc the white-on-color `CalenChatIcon` stands in) — plus the
words, because a corner affordance needs naming where a card did not. The form
is the screen's job; Calen is an accelerator, so it takes a corner rather than
permanent vertical space.

The eight forms: event, contact, maintenance task, chore, item, trip, trip
booking, recipe.

- **The pill hides while the keyboard is up.** It is anchored to the window
  bottom, so it would otherwise render on top of the keys — and the user is
  mid-typing in the form anyway. It fades (never unmounts, which would re-run
  the discovery pulse). Forms give their scroll content `fs.withFloatingPill`
  (bottom padding 96) so the last row clears it.
- **The sheet sizes to its content.** A fresh conversation is just the title
  strip and the composer (the composer's placeholder carries the example prompt
  — there is no separate hint line to duplicate it); it grows with the
  transcript and caps at 58% of the window, clamped to
  `winH − insets.top − keyboardHeight − spacing.lg` so a long conversation
  still fits with the keyboard up on an SE. A fixed height was tried first and
  rejected: it opened with a block of dead space between title and composer.
  Transcript above, composer pinned below — the inversion that keeps this clear
  of the "a search field never lives in a bottom sheet" rule, which is about
  results rendering *under* a field and therefore behind the keys.
- **A turn that fills the form closes the sheet after a ~900ms beat** — long
  enough to read Calen's one-line confirmation before the form returns with its
  changed fields highlighted (the highlight, via `useFormAssist`, is the durable
  record). **A turn that fills nothing leaves the sheet open**, so a clarifying
  question can be answered in the same breath.
- **The conversation lives and dies with the screen.** It is plain component
  state — never `lib/chatHistory`, never AsyncStorage, never resumable from the
  Recent-chats sheet, and never written to disk (form values stay in memory,
  matching the E2EE posture). A form stays mounted while a sub-screen (Location,
  Repeat, Alerts) sits on top, so the transcript survives a drill-in and return;
  saving pops the form and takes the transcript with it. The two forms that
  **re-seed themselves in place** — a trip booking imported from a confirmation,
  a recipe imported from a URL/photo — call `reset()` on the component, because
  what was said before is about a different record.
- **`attention`** pulses the pill once on mount, for arriving at a form whose
  expected next step *is* the assistant (a just-imported booking or recipe).
  Deliberately not an auto-opening sheet, which hijacks a screen the user has
  only just landed on.
- The `<CreditsBanner/>` low-balance heads-up renders **inside** the sheet, above
  the transcript — where the spend happens, not permanently atop eight forms.
- The section accent tints the send button only; the sheet chrome stays
  app-primary (it is Calen's, not the section's).
- **The master AI switch still governs.** With `aiEnabled` off, neither pill nor
  sheet renders and nothing is ever sent to the provider.
- The recipe form uses the same pill and sheet with a custom submit (`onSubmit` →
  `/edit-with-ai`) instead of the `/form-assist` fill: it rewrites the whole
  recipe and repopulates the form, so there is no patch, no field highlight, and
  the sheet stays open reporting what happened.

**The endpoint is conversational.** `POST /form-assist` accepts either the
legacy one-shot `prompt` or a `messages` transcript, and answers
`{ patch, reply, note, creditsUsed }` (`note` aliases `reply` so an app build
still running the old card keeps working; drop it a release after the pill
ships).

- `normalizeHistory` bounds the transcript to the last **8** turns, merges
  consecutive same-role turns (the API rejects them), and guarantees it opens on
  a user turn *after* trimming. The client trims identically
  (`lib/formAssistTranscript.ts`) so the user isn't billed for tokens the server
  is about to discard, and it never sends the UI-only `patch`/`applied` fields.
- The **live form values ride on the newest user turn only**. That is truer than
  replaying past patches: the user may have hand-edited a field, or undone a
  fill, in between.
- **`tool_choice` is the gate.** A legacy `prompt` body keeps the mandatory
  `{ type: 'tool', name: 'fill_form' }` — there is no conversation for a question
  to land in, so a text-only reply would be a dropped request. A `messages` body
  relaxes to `{ type: 'auto' }`, and the appended chat rules bias hard toward
  filling: call the tool whenever anything is fillable and write one short
  sentence naming what was set; ask **one** short question only when genuinely
  unable to proceed. A tool call that emits no text falls back to naming the
  changed fields from the patch itself.
- **Form assist is token-priced like chat.** It meters under the `chat` action
  and now calls `recordChatCredits` with the turn's usage breakdown, returning
  `creditsUsed` — it previously debited nothing at all, because `meter()` skips
  the flat debit for token-priced actions and this route never picked it back
  up. Pricing model: billing-plans.md.
- It stays **non-streaming**. The payoff is the form's fields lighting up, not
  text appearing, and `services/aiUsage.js` patches only non-streaming
  `messages.create` — a streaming form assist would silently record zero tokens.
...
- AI is **consent-gated**: the `aiEnabled` / `aiUsePersonalInfo` prefs hard-gate
  ...
  prompt: the calendar assistant omits the contacts roster, form-assist omits its
...
- Payloads are also **query-scoped** (Signal-parity G4): the calendar assistant
  ...
  the full history.
- **Friends & family are name-only.** For contacts of type `family`/`friend`, no
...
- **Rosters and record bodies are fetched on demand, not front-loaded.** The
  calendar system prompt contains no contacts; the model calls
...
- **The chores assistant drafts the whole chore form.** Its
  `open_create_chore_form` tool covers every field the form carries: title,
  instructions, **icon** (an enum mirroring `ChoreForm`'s glyph list),
  frequency + interval, **`firstDueDate`** (the first occurrence, YYYY-MM-DD
  resolved against today — sent whenever the user implies a start day or
  weekday, since for an interval series the anchor date IS the pattern day;
  without it "every 2 weeks starting this Sunday" seeds interval-from-today on
  the wrong weekday), **assignee**, and the **alert settings**
  (`reminderDaysBefore`/`alert2DaysBefore` from the form's day offsets,
  `reminderTime` HH:MM, `alertAudience` everyone/owner). The client passes the
  draft through as `ChoreForm`'s `prefill`, and the form validates each value
  against its own pickers (unknown icons and off-list alert offsets are
  dropped, not saved). The system prompt directs the model to always pick an
  icon but set assignee/alert fields **only when the user asked** — never
  invented defaults. Token discipline: all fields ride the single existing
  tool call (no extra round trips), the enums live in the prompt-cached
  system+tools prefix (cache-read after the first turn), and **no member
  roster is sent to the model** — `assignedToName` is just the name the user
  typed echoed back, matched to a household member on-device once the form's
  member options load (one shot; exact label match, else first-name match,
  "(You)" suffix ignored). Because the model never knows the user's own name,
  "assign it to me" arrives as a self-reference ("me"/"you"/"myself"), which
  the matcher resolves to the signed-in user's "(You)"-suffixed option.
...
- **References, not values.** Phone numbers and booking confirmation codes never
  ...
  - **The one carve-out: reading a confirmation the user hands over.**
    `POST /trips/:id/items/from-confirmation` (trips.md → "Add from a
    confirmation") sends the WHOLE confirmation — email body, e-ticket PDF or
    photo — to the model, because reading it is the entire request; a
    minimized payload would have nothing left to parse. The confirmation code
    the rule above keeps out of context comes back **from** this call, and
    from nowhere else. What keeps it inside the policy rather than an
    exception to it: it runs only on an explicit per-import gesture (never
    ambiently, never over stored bookings), the request is
    parsed and dropped rather than stored, the master AI switch removes the
    card entirely, and nothing about the parse is written back into chat
    context. **It is NOT disclosed in the card** — an in-card notice shipped
    and was later removed (trips.md → "Add from a confirmation", and its open
    question); the master AI switch and the credit debit are the only signals
    the user gets. A confirmation the user did NOT hand over this way stays sealed
    and stays "on file".
