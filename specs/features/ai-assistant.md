...
tests:
  ...
  - mobile/src/lib/__tests__/{aiPayload,aiWindow,chatHistory,chatLinks}.test.ts
...
- **Home area is coarse, not the street address.** The calendar assistant's
  ...
  Account screen. It is stored **plaintext** like `timezone` — coarse enough to
  be non-sensitive, and readable by the server so the cloud model can use it.
