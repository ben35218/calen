...
- **Details** — the full month grid: event **chips** (title + start time),
  ...
  **spanning bars take the same tinted treatment** (translucent wash, label in
  ...
  colour** — the day view's block edge — which is what distinguishes a
  ...
  **"+N more"** label beneath them, always on a single line — the row-height
  math reserves exactly one line for it, so on narrow cells the label shrinks
  to fit rather than wrapping (a wrapped second line would fall below the
  reserved height and clip). The remaining items are reached by tapping the
  day. Same rule in `ViewerMonthGrid`.
...
- **Who edits the calendar record (the Edit Calendar form).** The creator
  ...
  edit the calendar's **basics — name, colour, and the Alerts switch —**
...
- **A WidgetKit "Today" widget** (small / medium / large;
  ...
  effective calendar colours, calendar-visibility toggles, custom-calendar
...
- **Presentation is Apple-Calendar-widget style.** Accent weekday eyebrow over
  ...
  events as two-line calendar-colour-tinted chips (colour bar + title +
  locale-aware compact time range, e.g. "4–5 PM", all in the calendar
  colour); all-day items wear the SAME tinted-chip dress on ONE line — title
...
- **Adoption surfaces (the widget must be found to be used — iOS offers no
  ...
  **Profile promo card** (iOS only) — the widget is something the user does
  ...
  widget (today's weekday eyebrow + day number + colour-tinted chips, drawn
...
- **The widget snapshot is a deliberate device-local plaintext exception.**
  ...
  widget draws: titles, clipped minutes-from-midnight times, colours, a
