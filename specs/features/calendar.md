...
- **Bottom-right** — the standalone **Calen FAB**: the gradient "C" glyph on a
  ...
  **Until the first-ever tap** a soft **discovery halo** pulses from the disc
  ...
  was tried first and rejected: it collided with the Today | Calendars pill
  on small (SE-width) screens. (When the calendar
...
- **Single Day / Multi Day (the timeline modes)** are one hour-grid surface
  ...
  - **Long-pressing empty grid space starts a new event there** — the month
    ...
    **15-minute slot under the finger** and opens the event form as a timed
    ...
    responds — event blocks own their own touches (tap → detail, unchanged),
    and a scroll or day-swipe cancels the press like any touchable. In Multi
    Day each column drafts on its own date.
