...
code:
  ...
  - mobile/src/lib/inviteeDraft.ts         # one form session's invitee staging: adds, revokes, household picks, guest-list flag
  - mobile/src/lib/widgetSnapshot.ts       # home-screen widget: 14-day App Group snapshot writer
  - mobile/src/hooks/useWidgetSnapshot.ts  # mounts the snapshot writer (sign-in / foreground / data change)
  - mobile/modules/calen-widget/           # native bridge: protected App Group write + WidgetKit reload
  - mobile/targets/widget/                 # the WidgetKit extension itself (SwiftUI timeline + views)
...
tests:
  ...
  - mobile/src/lib/__tests__/{calendarData,calendarFeeds,calendarPrefs,calendarWindow,monthGrid,holidays,homeRegion,weatherSource,recurrence,tz,printCalendar,addons,occasions,recordStore,eventWhen,occurrenceShift,eventSave,eventAlerts,householdRsvp,recordSocket,eventInvitations}.test.ts
  - server/src/test/recordPoke.integration.test.js
  - mobile/src/lib/__tests__/calenFabIntro.test.tsx  # Calen FAB first-run "Ask Calen" pill one-shot
  - mobile/src/lib/__tests__/widgetSnapshot.test.ts  # widget snapshot: window shape, routing, bridge wiring
  - mobile/src/navigation/__tests__/rebindDetailBelow.test.ts
  - mobile/src/screens/calendar/__tests__/CalendarsScreen.test.tsx
  - mobile/src/screens/calendar/__tests__/weatherLocationSearch.test.tsx
  - mobile/src/screens/calendar/__tests__/EventInviteesScreen.suggestions.test.tsx  # invite field: one row per contact address
  - mobile/src/screens/calendar/__tests__/EventInviteesScreen.stageOnly.test.tsx    # invitee edits stage until the event form's save
  - mobile/src/screens/calendar/__tests__/EventDetailScreen.alerts.test.tsx         # alerts managed in place on the event view
  - mobile/src/screens/calendar/__tests__/sealedEventFlips.test.ts                  # sealed single-field/alert writes go through the re-seal lane
...
- **Discard-changes guard:** leaving a form with unsaved edits — via the header
  ...
  E-Card screens, and app-wide on the Contacts, Account, Chore, Task, Item, Trip,
...
- **Starts / Ends editing:** the shared date/time picker (`DateField`/`TimeField`)
  ...
  button all commit it; there is no discard-on-tap-away. The **minutes wheel
  steps in 5-minute intervals** (Apple Calendar's granularity — a flick snaps to
  a clean :05 boundary), and that is `TimeField`'s app-wide default, so every
  time-of-day picker (event Starts/Ends, chore/task due times, holiday/occasion
  alert times, the day-alert reminder, booking times, Calen call windows)
  inherits it. The one opt-out is minute-precise timetable times — the trips
  spec's flight/transit Departs/Arrives (`minuteInterval={1}`). A stored
  off-grid time (e.g. an imported 7:43) is *not* re-rounded by opening the
  sheet: dismissal commits the untouched seed verbatim; only spinning the wheel
  lands on the 5-minute grid. The two endpoints
...
- **Recurrence** supports `daily` / `weekly` / `monthly` / `yearly` with an
  ...
  (`shared/calendar/`), so mobile and any other consumer agree. Ordinals run
  first..fifth plus last (-1) and next-to-last (-2); a month without the nth
  occurrence (no fifth Friday) simply doesn't emit — for events and, since the
  `nthWeekdayOfMonth` fix, identically for the chore/task interval path
  (semantics owned by [maintenance.md](maintenance.md)). The pushed Repeat
  screen (`EventRepeat`) is shared with the chore/task forms, which pass a
  `singleDay` route param restricting it to what their sealed model stores
  (single weekday / month-date, concrete Sun..Sat kinds); **without the param
  — every calendar-event entry point — the picker is unrestricted** and
  multi-select works as it always has. On the chore/task side, the bridge
  (`recurrenceToRule`/`ruleToRecurrence` in `lib/recurrence.ts`) carries a
  yearly recurrence's anchor `dayOfMonth` through the rule's `daysOfMonth`
  slot in both directions, so a calendar-type "Mar & Sep on the 15th" survives
  an edit round-trip instead of falling back to the 1st — behavior owned by
  [maintenance.md](maintenance.md); events are unaffected (their day comes
  from the start date, and the yearly picker writes no `daysOfMonth`). The **End Repeat**
  ...
  **detail view** renders the recurrence as a summary line under the date/time
  ...
  appended when an End Repeat is set — accent-colored, mirroring the form's
...
- **Occurrence scoping re-seals in the record's own key lane.** An event on an
  ...
  **same prompts as any other**; withholding the capability from them (the earlier
  ...
  `cancelEvent`, and `setAlerts` (the event view's in-place alert pickers) route
...
- **Reminders/alerts:** up to two alerts per event (`reminderMinutes`/`At`,
  ...
  alerts — an "Alert" row plus a "Second alert" row (the latter only when a first
  alert is set) **grouped in one hairline-divided card** (Apple Calendar-style),
  matching the form's Alert / Second Alert fields. The two
  ...
  filtered out.
- **The event view's alert rows are live pickers — an alert is managed without
  opening the edit form.** Tapping either row opens the same option sheet the
  form opens (including **Custom…**), and the pick is **written immediately** to
  the event; there is no save button and no scope prompt. Setting, changing, or
  clearing an alert on an event that already exists is the single most common
  reason to reopen one, and routing that through the edit form — open, scroll,
  pick, save, answer the repeating-event scope sheet — was the long way round to
  a one-tap change. The rows the two surfaces offer are built by **one shared
  builder** (`lib/eventAlertOptions.buildAlertItems`), so the event view can
  never drift into offering a different list than the form for the same event —
  the departure-anchored rows, the day grid on an all-day event, the synthesized
  row for a saved custom value, and the "the other slot used this" filter are all
  the form's, unchanged. Every rule the form's pickers obey holds here: clearing
  the first alert **promotes the second**, the second row hides while no first
  alert exists, the chosen **anchor** is stored beside the minutes, and a
  dismissed custom sheet commits what its wheel is showing.
- **The write is the sealed re-seal lane, not a plaintext field update.** Both
  slots travel as ONE patch (`calendarApi.setAlerts` → `resealInLane`), which
  merges them over the decrypted replica record and re-seals the whole
  `EVENT_ENC` subset under **whichever key the event already lives under** — an
  event on an outside-shared calendar keeps its CalendarKey lane instead of being
  flipped to the household key, which would lock its collaborators out. The pair
  is written together because it is *set* together: clearing the first slot
  promotes the second, so a per-slot write could leave the record holding a
  second alert with no first — a state the promotion rule exists to forbid. A
  cleared slot is sent as `undefined`, which drops the field out of the sealed
  blob. The row shows the pick immediately and the write happens behind it; a
  failed write **puts the row back** to what the event still says and reports it,
  rather than leaving a setting on screen that was never stored. Success
  invalidates `['calendar']`, which is what re-runs the on-device reminder
  schedule (`useReminderScheduler`) and so makes the new alert real.
- **On a repeating event, alerts belong to the series, and the row says so
  before the tap.** A repeating event is one record, so there is no
  per-occurrence alert to set — the detail view's rows have always *displayed*
  the series' alert. The card therefore carries the line "Alerts apply to every
  occurrence of this event." whenever the event repeats. It is stated up front
  rather than confirmed afterwards because the user is looking at a single day
  and could reasonably read the row as belonging to it. Scoping an alert to one
  occurrence means **detaching** that occurrence into its own record, which is a
  structural change an in-place picker must not make silently; the edit form,
  with its "Save for This Event Only / Save for Future Events" sheet, remains the
  place for it.
...
- **Travel time** (`travelMinutes`, `travelDistanceKm`, `travelMode`) may be
  ...
  **never changes its travel time automatically** — neither the auto-on default
  ...
  (`colors.primary + '22'`, primary-colored glyph) and the rest bare
  text-colored glyphs — no borders, no labels; each button carries an explicit
...
- **Cancellation via AI call:** when Calen's cancellation call gets a business to
  ...
  **prominent callout banner at the top** — a tinted box (the app's banner
  ...
  banner is tinted with the **event's own calendar color** (the calendar whose
...
- **Detail view reflects every configured field.** The guiding principle: whatever
  ...
  the event drawn as an accent-tinted block — solid accent left bar, accent-color
...
- **Compact** — a uniform short row per week; each day shows the day number and a
  row of up to four colored **dots** (one per source: each spanning span
...
- **Stacked** — each single-day item renders as a thin **colored bar** (no
...
- **Details** — the full month grid: event **chips** (title + start time),
  ...
  background is the calendar's color at low opacity over the black canvas and
  its title and start time are drawn **in that color**, not in white. The text
  color is derived, never the raw stored hex — `lib/color.tintedChip` lightens
  ...
  so a dark calendar color stays legible instead of reading as a smudge. The
  **spanning bars take the same tinted treatment** (translucent wash, label in
  the corrected calendar hue) plus a **solid left edge bar in the raw calendar
  color** — the day view's block edge — which is what distinguishes a
  multi-day span from a single-day chip at a glance. (Stacked's unlabelled
  span bars stay solid, matching that density's solid single-day bars.)
  A **trip's** bar — and its day dot, its List-mode row, its day-view all-day
  chip and its search result — is painted by the **Trips calendar's** color
  like any other item is painted by its calendar's: a trip carries no color of
  its own, so recoloring that calendar recolors every trip
  (see [trips.md](trips.md)).
  **A spanning bar's tap is day-aware** (tapped column → date via
  `locationX ÷ cellSize`, clamped to the bar's own columns): a multi-day
  **event** bar opens the event view seeded with the tapped date, and a
  **trip** bar opens that trip **on the tapped day** — the `TripDetail`
  route's optional `date` param lands the screen directly in the day
  itinerary for that date (see [trips.md](trips.md#day-itinerary-the-trips-own-calendar--normative)),
  with the trip's own back chevron surfacing the overview grid; a date the
  trip's day list doesn't contain just opens the overview. Long-press still
  edits (event form / trip form). The List mode's trip row behaves the same:
  it sits under the chosen day, so it passes that day as `date`.
  `ViewerMonthGrid` (free viewer mode) renders the identical chip and bar.
  **A spanning bar's lane is exactly a one-line chip's slot** (same body height,
  same gap): a multi-day event sits in the same vertical rhythm as the chips
  beside and below it, rather than in a tighter one that reads as a lesser row.
  The **forecast strip is not an event** and keeps its own shorter lane (it
  carries a glyph and a 9pt temperature, not a title), so turning the weather on
  doesn't inflate every week.
  A day cell shows at most **three** chips; the overflow collapses into a
  **"+N more"** label beneath them, always on a single line — the row-height
  math reserves exactly one line for it, and a wrapped second line would fall
  below the reserved height and clip. That label is drawn in the **chip's own
  type**, muted — it is an event line that didn't fit, not a footnote — and is
  **never auto-shrunk**: a cell too narrow for the full label drops the word
  ("+2") rather than the point size, so it can't render smaller than the chips
  it summarizes. The remaining items are reached by tapping the day. Same rules
  in `ViewerMonthGrid`.

  **A cell fits its items to the row it got; the row never grows to swallow
  them.** A week is sized by its busiest day, then **clamped at `maxWeek`** — so
  on the busiest days the ask exceeds the row, and the cell's `overflow: hidden`
  would otherwise saw whatever is last in flow (the icon row) in half. Instead
  the cell measures itself against the space it actually has
  (`lib/monthGrid` → `cellItemSpace` + `fitCellChips`, shared by both grids) and
  gives up **whole chips**, in this order of precedence:
  1. **The icon row is reserved first.** It is the day's one-line summary of
     everything that isn't an event (maintenance, chores, meals, grocery,
     occasions) and it renders last, so it is the thing that clips. A day gives
     up a chip before it gives up its glyphs, and a half-drawn glyph row is a
     defect. **That row is one line, always** — it never wraps, because a second
     line falls outside the single reserved line and gets sheared off the same
     way. It measures its glyphs against the cell's width and renders as many as
     fit (`fitIconRow`), dropping the tail in the order occasions → maintenance
     → chores → meals → grocery. Nothing is lost: the cell's own tap opens the
     day, which lists every one of them.
  2. **The "+N more" line is reserved before the last chip kept**, so a dropped
     chip is always accounted for on screen.
  3. Whatever is left goes to chips, longest-prefix-first.
  A cell also gets back the space of the bar lanes it does **not** sit under, so
  a quiet day beside a trip isn't charged for the trip's lanes. Row heights are
  unchanged by this pass — only the busiest, already-clamped days show fewer
  chips (and a correspondingly larger count) than the three-chip cap.
- **List** — a compact single-month grid (dots per day, like Compact) with the
  **tapped day's events listed below** (as compact cards). Only the visible
  ...
  any other day demotes today to a bare primary-colored number (no disc) and
  the tapped day carries a **white disc with the number knocked out** (rendered
  in the screen's black canvas color, so the number reads as the background
  ...
  showing — with today selected and circled in the primary color when that
...
carries the abbreviated month name in the app **primary** (16pt), on a reserved
line above the day numbers — and **above the month-opening rule**, Apple
Calendar-style: the name sits on top of the line, with the line between it and
the day numbers. The rule that opens a month's first row is the **same hairline
in `colors.border` as every other week rule** — not a tinted or heavier line —
drawn as the **bottom border of each own-month cell's label slot, only over the
days the month owns**: the blank cells leading into the 1st get no rule, so the
line starts at the 1st instead of hanging over the whitespace. This holds at
every density in the family (Compact, Stacked, Details) and in free viewer
mode's `ViewerMonthGrid`, which carries the identical layout.
...
together when the underlying data changes. The by-date item buckets feeding the
cores are their own memo, keyed on the data alone, so a density switch doesn't
re-scan every event in the window on its way to a wall of cache hits. What a
switch *does* still pay for is re-rendering the mounted week rows for the new
layout, so the grid list caps its render window (`windowSize={7}` — about three
screens of runway per side, against React Native's 21-screenful default, which
with ~48pt Compact rows kept hundreds of rows mounted and made the switch
visibly drag). And the switch itself is a **React transition**
(`startTransition`): the switcher popover's dismissal commits urgently and
paints first, while the grid re-renders interruptibly behind it — the current
density stays up and responsive until the new one is ready, so the menu never
hangs on the way out (this replaced an earlier one-frame
`requestAnimationFrame` deferral, which unblocked the popover but still froze
the thread for the full re-render a frame later).
...
- **A month's expansion chunk covers its last day to the last instant**
  (`monthRange` ends at 23:59:59.999 local, not local midnight). The engine
  keeps an event only while `startDate <= to`, so a midnight bound orphaned
  every timed event on a month's final day — it started after its own month's
  bound and ended before the next month's `from`, so no chunk claimed it and
  the merged grid silently dropped it (while the day view, whose ±7-day window
  is not month-aligned, still showed it).
...
- **Bottom-right** — the standalone **Calen FAB**: the gradient "C" glyph on a
  56pt disc, larger and one shadow step above the pills, alone in the prime
  thumb corner. It is the screen's single visually-dominant primary action;
  shown only while AI is enabled. The disc holds its edge with an **elevated
  fill + faint light rim** (`colors.surfaceElevated`/`colors.outline`) — the
  canvas is pure black, where a black drop shadow separates nothing, so
  elevation is carried Material-dark-style by the surface itself, never by
  shadow alone. Pressing it **scales the disc down with a spring** (~0.92,
  releasing with a soft bounce) and fires a light haptic; it is announced to
  VoiceOver as a button ("Ask Calen", hint "Opens the Calen assistant").
  **Until the first-ever tap** a soft **discovery halo** pulses from the disc
  — the primary-blue footprint scaling up and fading out (peaks at 0.4
  opacity), a bounded 6 pulses per mount, never under iOS Reduce Motion — the
  bare "C" is brand, not affordance, so the pulse says "this one is tappable"
  without taking layout room; after the first tap it is retired for good
  (device-local `lib/calenFabIntro`; the month grid and day view share the
  flag, so tapping either settles both). An extended "Ask Calen" label pill
  was tried first and rejected **here**: it collided with the Today | Calendars
  pill on small (SE-width) screens. That was a crowded-corner problem, not a
  pill problem — the same `AssistantButton label` variant is exactly what the
  add/edit forms float, where the corner is empty (see
  [ai-assistant.md](ai-assistant.md)). (When the calendar
  was pushed by an assistant nav chip, this slot shows the "‹ Calen" return
  pill instead — see [ai-assistant.md](ai-assistant.md).)
...
2. **The screens swap while both are empty.** The month animates out *first*
   ...
   color plus that identical bottom chrome, so the hard cut has nothing to
   show.
...
- **A record write invalidates the calendar — at the CRUD chokepoint, not per
  ...
  `Chore`, `Contact`, `RecipeSchedule`), coalescing bursts (a bulk contact
  ...
  kept the pre-edit dates until the next sync pass, because the contact form
...
- **Single Day / Multi Day (the timeline modes)** are one hour-grid surface
  ...
  - A **week strip** (weekday letters + paging date numbers) sits above the
    grid. Today is marked in the **app primary color** (never Apple's red):
  ...
  - The **hour grid** is a fixed 24h canvas (1 px/min), gutter labels
    ...
    calendar-color fill, solid color bar on the left edge, text in the
    calendar color (contrast-corrected by `lib/color.tintedChip`, the same
  ...
  - An event with a **drive time set extends upward from its start**: the
    ...
    band** — the same calendar color at a third of the block's tint, a faded
  ...
  - The **all-day lane** (hidden when every visible day is empty) holds
    ...
    MaterialCommunityIcons glyph tinted in the item's color: **events** a
    ...
    calendar's color** (`calColors.recipes`, user overrides included) like
    those other surfaces; the shopping day belongs to that calendar, so a
    color of its own would read as a separate calendar. A meal chip is titled
    with the **recipe's own name** (see the title join below), never the word
    "Recipe".
    Date-only **tasks** render as muted empty-circle chips (not a color-badged
    item). Capped at three rows per day with "+N more" expanding.
  - The **now indicator** renders only when today is visible, in the app
    primary color: a line with a dot across today's column (dimmer across
  ...
  - The **hourly weather rail**: while the Weather calendar is toggled
    ...
    no rail.
  - **Long-pressing empty grid space starts a new event there** — the month
    grid's long-press-a-day, with the time filled in. The press maps to the
    **15-minute slot under the finger** and opens the event form as a timed
    draft: all-day **off**, start at that slot, **one hour long** (Apple's
    default duration); a press in the last hour keeps its slot and rolls the
    end past midnight onto the next day's date. Only the empty canvas
    responds — event blocks own their own touches (tap → detail, hold → edit;
    next bullet), and a scroll or day-swipe cancels the press like any
    touchable. In Multi Day each column drafts on its own date.
    The press is answered **visibly, not just by navigating**: in sync with a
    medium haptic, a **ghost "New Event" block springs into the snapped slot**
    (scale ~0.95 → 1 with a fade, the standard event-block look in the
    app-primary tint — the draft has no calendar yet), titled "New Event" with
    its start–end range. It **holds a beat (~250 ms) before the form pushes**,
    so the eye registers where the draft landed, and it stays on the grid
    beneath the pushed form — reading as the event being created. When the day
    view regains focus the ghost **fades out**: the saved event takes its
    place, or nothing does if the form was cancelled. A last-hour ghost clips
    at the column's end like any block while its range line still names the
    midnight-crossing end.
  - **Tap to read, hold to edit — the month grid's gesture vocabulary,
    answered on every day-view item** (same `LONG_PRESS_MS` ≈ 200 ms hold,
    `lib/calendar`). A tap opens the item's detail; a hold opens its **edit
    form directly**: an event block or chip → the event form (the held day
    rides along as the return date), and in the all-day lane a trip → the trip
    form, a task/chore → its form **scoped to the held occurrence** (the day
    goes with it, like the detail tap), a meal → the recipe form
    (`dayview/dayNav.editAllDayItem`, the hold counterpart of
    `openAllDayItem`). Kinds with no edit form from this surface (occasions,
    holidays, the grocery marker) **fall back to their tap action** — a hold
    is never a dead press. A block's hold claims the touch, so it never falls
    through to the canvas's new-event draft underneath. (±1 single, ±2 multi) with the
...
- **List** — a continuous agenda of **days with items only** (plus **today**,
  ...
  divider marker** (accent lines + label, app primary color) and its date line
  is tinted primary — the same today-marker idiom as the Occasions view. Rows
  are led by a color-tinted glyph rather than a bare color bar: timed +
  ...
  **chores** the chore's own icon; date-only **tasks** stay **muted
  ...
  start/end times on the right; all-day items are marked "all-day". Rows
  answer the same **tap to read / hold to edit** pair as the grid family's
  blocks and lane chips (above). The window
  **starts at the
...
- A household may create custom calendars (color + name); these are managed
  through `server/src/routes/calendars.js` (`/api/calendars`) and the mobile
  Calendars / Add-Calendar / Calendar-Colors screens.
- **Who edits the calendar record (the Edit Calendar form).** The creator
  manages **sharing** (household toggle & access level, member picks, outside
  shares), the **holiday config**, and **deletion** — sharing edits drive
  invitations and CalendarKey wraps that only the owner's device can perform.
  A **housemate holding Full Access** (household-wide or member-specific) may
  edit the calendar's **basics — name, color, and the Alerts switch —**
  matching what Full Access already means for its events; their form shows the
  sharing sections read-only with a note ("Sharing is managed by its owner"),
  keeps Delete hidden, and saves a basics-only payload. A **view-only**
  housemate reads everything (no save check, "only the calendar's owner can
  make changes"). An **outside collaborator never edits the calendar record**,
  even at Full Access — their access is to the events. The server enforces the
  same split on `PUT /calendars/:key`: the calendar resolves through the
  requester's access filter (404 when not visible), a full-access
  same-household member may update `name`/`color`/`alertsEnabled` only (403 on
  a payload touching sharing or holiday fields), everyone else 403s. The
  client keys its gate off `CustomCalendar.ownerId` (the creator's userId,
  compared against the household member list) + `access === 'full'`.
- The **name** is the only typed field on the New Calendar and Subscribe
  screens (sharing, color and alerts are all tapped), so its keyboard **Done**
...
- **Holiday alerts are one config for every holiday calendar.** The holidays
  ...
  the Occasions view uses, tinted with the calendar's color) opening **Holiday
...
- **Presentation: a push**, not a modal. The manager is browsable hierarchy —
  it drills into Add Calendar, Edit Calendar, Colors & Order, and Print — so it
...
- Calendars group by **audience**, ordered **HOUSEHOLD** (built-ins +
  ...
  **SHARED** by default, and in whatever sequence the user arranged in
  **Colors & Order** when they have (`User.calendarPrefs.groupOrder`; a group
  they never moved trails the ones they did, in the default order);
  empty groups are hidden. Within a group, built-in and custom calendars are
  **one sequence** ordered by the user's arrangement (built-ins lead only the
  natural order), not a built-ins block followed by a customs block. Every row
  in SHARED MUST state its
  direction in the subtitle — "Shared by you · N contact/contacts" (member +
...
- Rows are **single-purpose**: tapping a row toggles that calendar's
  ...
  **leading control is an Apple-style on/off circle** (replacing the old accent
  bar) carrying the calendar's color — a filled check-circle when shown, an
  empty outline circle (dimmed) when hidden — so the toggle state and the
  calendar's color read from one control at the start of the row. Tapping a row
...
- **Hiding a calendar hides it on EVERY surface.** The toggle is a display
  filter over the whole app, not a month-grid setting: the month grid *and* its
  List mode (both the day-cell dots and the selected day's rows) *and* all
  three day-view modes (Single Day, Multi Day, List) omit a hidden calendar's
  items entirely. The mapping from a fetched collection to the calendar that
  owns it lives in one place — `lib/calendar.visibleDayItems`, which every
  `itemsForDate` consumer pipes through: events by their own `calendarType`,
  tasks → Maintenance, chores → Chores, recipes **and the grocery-shopping
  marker** → Meals, trips → Trips, occasions → Occasions, holidays by their
  per-country calendar, the weather rail/glance by Weather. Re-deriving that
  mapping per screen is what let toggled-off calendars keep showing in the day
  view and in the month's List rows (both filtered only their dots), fixed
  2026-08-12.
- Built-in calendars ship with default colors (overridable via Colors & Order,
...
- **The arrangement belongs to the ACCOUNT, not the device.** How the user
  arranged their calendars — each one's **color**, the **order** they list in,
  the **order their audience sections list in**, which are **hidden**, which
  built-ins they **deleted**, and which have **event
  ...
  state wiped at sign-out (it holds one account's calendar names, colors and
  ...
  2026-08-04 against a recolored Chores calendar. The rules:
  ...
  - On load, the cache paints first and the account's arrangement is then
    ...
    **never stored** means this device's own choice becomes the account's (the
    upgrade path for a user whose colors predate this); a field the account
  ...
  - **The first frame carries the user's colors.** Every calendar surface
    resolves color through `lib/calendarPrefs` (`colorOf` /
    ...
    chips, dots and section accents in the DEFAULT colors and recolors them a
    beat later — reported 2026-08-04 ("it displays the events and icons in their
    default colors for a second or two"). Two rules keep the first paint
    ...
    **holds its splash on `useCalendarPrefsReady`** while signed in, alongside
    ...
    bounded wait rather than a stuck splash or a wrong-colored calendar.
...
- Navigation lives in the explicit trailing controls: **every row carries an
  ...
  path to name/color/alerts/sharing/delete. Rows with a content view
  ...
  **no header edit pencil** — the row's edit button is the single edit path.
  ...
  Holiday Alerts), tinted with that calendar's color.
- The primary **add action is the header `+`** (app convention), opening the
  ...
  Secondary actions — Calendar colors & order, Print — are one grouped
...
- **It shows the same calendars, in the same sections, as the Calendars
  manager.** Both screens read one grouping (`hooks/useCalendarGroups`), so the
  list here holds **every** calendar the household has — built-ins, hand-made,
  subscribed and holiday calendars alike — under the **HOUSEHOLD / JUST ME /
  SHARED** headers, with the same solo-household merge and the same exclusions
  (locked add-ons, deleted built-ins). Deriving the list separately is what let
  the screen fall behind the app: it knew only the built-ins and the holiday
  calendars, so a calendar the user created or subscribed to never appeared and
  could be neither recolored nor placed (reported 2026-08-22).
- **Three things are arranged here, and all three are account state** (see the
  arrangement rules above): a calendar's **color**, its **position within its
  section**, and the **sequence of the sections themselves**.
  - Tapping a calendar opens its palette (a superset of `ColorPicker` — the
    presets plus **Reset**, which drops the override back to the calendar's own
    color); the pick applies app-wide when the panel closes.
  - The up/down chevrons on a row move a calendar **within its section only** —
    sections are audiences, so a calendar can't leave one by being nudged past
    its edge. The write is the whole displayed id sequence, section by section
    in their displayed order, with ranks for calendars this screen doesn't show
    carried along at the end.
  - **A section is ONE sequence — built-ins and custom calendars are peers.**
    Built-ins lead a section's *natural* order (what an untouched account sees),
    but every row in a section sorts against every other, so a custom calendar
    can be placed anywhere among the built-ins and both lists render it there.
    Sorting the two kinds separately and rendering them back to back is what
    pinned a newly-added calendar to the bottom of HOUSEHOLD, where its "up"
    chevron and the built-ins' "down" chevrons both silently did nothing
    (reported 2026-08-23).
  - The up/down chevrons on a **section header** re-sequence the sections, and
    the Calendars manager lists its groups in exactly that sequence.
- **No hint line.** The chevrons and the swatch say what they do; a paragraph
  above the list was prose the user had to read past to reach the controls.
...
- `loadCalendarData` MUST exclude locked features' items (tasks, trips,
  ...
  checklist, the **Colors & Order list**, and the Add-Calendar menu's
...
- Locking never deletes anything: device prefs (visibility/color/order/deleted)
...
- **Nothing on the Invitees screen commits while the event form still can
  discard it.** Opened from the **event form** (new or saved — the form passes
  `stageOnly`), the screen's ✓ only writes the session's staging to
  `lib/inviteeDraft`: invitees added, invitations removed, the household
  selection, the guest-list flag. The **form's** save is the single place any of
  it happens — revokes first (so an address dropped and re-added in one session
  ends up invited), then the household push for members newly added, then
  `sendInvitations`. Opened from the **event detail screen** there is no pending
  save behind it, so its ✓ still commits on the spot; that is the only entry
  point that does. The rule is that an outsider must never be holding an
  invitation to an edit the organizer then backed out of — and, before this, a
  saved event's Invitees ✓ sent immediately while the edit behind it was still
  unsaved. Consequences that follow from it:
  - Removing an already-sent invitee **stages the revoke**: the row leaves the
    list at once (the user said remove), the `invitationsApi.revoke` waits for
    the save, and the confirm says so ("…when you save the event") rather than
    describing it as already done.
  - The Invitees row on the form previews what the event WILL have — everyone
    invited, minus the staged removals, plus the staged additions.
  - Staged work makes the form **dirty**, so backing out raises the discard
    prompt and discarding drops it; none of it lives in the form's own state, so
    each piece is compared against what the event seeded with. Every form
    session starts by emptying the staging area, so an abandoned draft or a
    discarded edit can never leak invitations into the next event's save.
  - A send that fails no longer disappears: the form reports which invitations
    didn't go out (the same treatment attachment-upload failures already got).
  - The per-row **Remind** / resend-text actions are unaffected — they compose
    from the organizer's own mail/Messages app on tap, which is the user
    sending, not the app committing an unsaved edit.
- **The "Invite others" field autocompletes over the contacts roster through the
  same shared `useRosterSuggestions` hook as the household invite and the
  calendar outside-share** (behavior specced in
  [households-sharing](households-sharing.md)) — the invite fields all resolve
  addresses identically, and this screen must not re-roll the matching. A matched
  contact therefore lists **every address it can be invited at** (each email,
  then each canonical-E.164 phone, shown as `label · address` and channel-
  iconed), so the organizer picks which one gets the invitation; tapping a row
  **stages** that exact address in **New** (nothing sends until the ✓ / the
  event's save). A taken address removes the whole contact from the dropdown,
  never just the address that collided — addresses already staged, addresses of
  outstanding/sent invitations for this event, and the organizer's own email and
  account phone (one exclusion set; the server rejects self- and re-invites
  anyway). The typed path is unchanged: any email or phone number still parses
  on return, whether or not it is on the roster. This field previously resolved
  each contact to its **primary email** (falling through to the primary phone
  only when the typed text looked digit-y), which hid every other address a
  contact could be reached at.
- **Household invitees (in-household accept/decline).** Housemates already see
  ...
  - The creator picks members in the Invitees screen's **"Notify household
    ...
    on edit, sealed into every save payload) so a whole-payload re-save
    preserves it. In a form session the selection lands with the event's save,
    which is also when the members newly added are notified (the ones already on
    the list are not re-notified — one save, one notification). Only the
    detail-screen entry re-seals it on its own, via
    `calendarApi.setHouseholdInvitees`, and notifies from there.
  ...
  - **Surfaces:** the invited member gets a push, a `householdEvent` row in
    ...
    the event, which is already on their calendar), and an **in-app pop-up on
    open** — the guaranteed surface when the push banner was denied or missed
    (`hooks/useInviteAlerts` + `lib/inviteAlerts`, spec:
    [notifications.md](notifications.md), In-app invitation pop-up). The
    creator gets the reply
...
- **The Invitees screen is three titled zones**, because notifying a housemate
  and inviting an outsider are different acts on different contacts and a single
  ...
  and **Guest list** (the visibility switch — a setting, not a contacts list, so
...
- `guestListVisible` controls whether cross-household invitees can see who else
  ...
  invitee draft store), so in a form session the switch lands with the event's
  save like everything else on that screen. From the detail-screen entry the
  toggle re-seals the event client-side via `calendarApi.setGuestListVisible` —
  never a plaintext field update, which the opaque store rejects on an E2EE
...
- **When the two households merge, the invitation is reconciled away.** An
  `EventInvitation` is a contract *between* two households — the organizer keeps
  their event, the recipient owns an independent copy, neither syncs the other's
  row. If those two people later join the **same** household that premise
  dissolves, and every surface built on it misbehaves: join carry-over
  ([households-sharing](households-sharing.md#join-carry-over-a-joiners-data-follows-them))
  hauls the recipient's copy in beside the original, so the same event shows on
  the calendar **twice** (one of them permanently read-only, wearing "Leave
  event"); the recipient is on no invitee chip because they were never in the
  event's sealed `householdInvitees`; and their accept/decline is stranded on a
  server row the calendar never reads. The reconciliation converges all of it
  onto the in-household representation above (`householdInvitees` + `EventRsvp`):
  - **The key is exact, not heuristic.** The invitation already carries
    `eventId` (the organizer's original) and `acceptedEventId` (the recipient's
    copy), so "which two rows are the same event" is a lookup. Events are
    **never** matched on title/time — there is no content-based dedupe anywhere
    in the app, and the server is content-blind besides.
  - **The recipient's device drives it** (`lib/invitationMerge`), from
    `maintainKeyHygiene` on every unlock, ordered **after** carry-over (the copy
    it resolves is one carry-over just moved, and the source event must be
    readable under the *current* HDK before anything can be re-sealed onto it).
    Recipient-driven for the same reason carry-over is: the server holds no key,
    and an `EventRsvp`'s responder identity is the C4 `author` folded inside the
    ciphertext, so only that person's device can author their own answer.
    `GET /invitations/reconcile` lists the rows whose organizer is now a
    housemate; the organizer's device is handed none of this work.
  - **Per invitation:** the recipient joins the source event's
    `householdInvitees` (a set-add — an existing invitee list is preserved);
    an `accepted`/`declined` answer is carried across as an `EventRsvp` with the
    original `respondedAt`, **without** re-firing the reply push (that reply was
    news when it was given, not on a house move); `POST /invitations/:id/merge`
    tombstones the now-duplicate copy and retires the row to the terminal
    **`merged`** status. A `pending` invitation simply becomes an in-household
    request (no answer to preserve — the inbox derives the Accept/Decline card
    from the event). A `left` invitation is retired *without* re-inviting them:
    leaving was a deliberate choice.
  - **The organizer's event is always the survivor.** The exception is an
    orphaned copy — the organizer deleted their original — where the copy is the
    only surviving record: it is **kept** and unlinked instead (its sealed
    `invitationId` cleared), promoting it to an ordinary editable household
    event rather than deleting the last trace of it.
  - **Sealed work first, retirement last**, and every step is idempotent (set-add
    invitee, absent-only RSVP, no-op detach, already-`merged` merge), so a pass
    interrupted anywhere is simply redone on the next unlock. A row whose records
    haven't reached the replica yet **defers** rather than acting: `resealInLane`
    rebuilds the whole sealed subset from the replica's copy, so re-sealing a
    record the device can't read would write an *empty* event over a real one.
  - **A `merged` row is inert history.** It is hidden from the recipient's
    Invitations inbox, from the organizer's `/sent` invitee list, and from the
    guest list — that person is a housemate now and shows as a household invitee
    chip. It is kept only as the audit trail of an invitation that predates the
    merge, and is never acted on again.
  - **The destructive lanes refuse for the window before the pass runs.** Once
    the two parties share a household, `POST /:id/leave` and the organizer's
    `DELETE /:id` both 409 — each would otherwise tombstone a record the
    household jointly owns (the recipient "leaving" would delete the event out of
    everyone's calendar). `POST /:id/accept` 409s too: it would mint a second
    copy of an event the recipient already syncs.
...
- **The Outside My Household add field autocompletes over the contacts
  ...
  the decrypted Contacts records matched by name, email, or phone digits via the
  shared `useRosterSuggestions` hook (behavior specced in
  [households-sharing](households-sharing.md)); placeholder "Add name, email,
  or phone…". A matched contact lists **every** address it can be shared with
  (each email, then each canonical-E.164 phone, labelled and channel-iconed);
  tapping one **stages** that address in the outside list at View Only — unlike
  the household invite, nothing sends until the calendar saves — then runs the
  same lookup-gated outreach as a typed add. A taken address removes the whole
  contact from the dropdown, never just the address that collided:
...
- **Forecast strip in the month grid.** When the Weather calendar is toggled
  ...
  Weather calendar's color) at lane 0 — event/trip bars stack below it — with
  one segment per forecast day (condition icon + high temp). Its lane is
  **shorter than an event bar's** (see "Details" above: a bar lane matches a
  one-line chip, the forecast lane sizes to its glyph + 9pt temp). It spans exactly
  ...
  Data via `loadCalendarForecast` (trip-aware + source-aware, never prompts; a
  failed load just means no strip). A day the household spends on a trip
  shows the **destination's** weather, its segment led by a small airplane
  glyph (see "Trip days show the destination's weather" below). Tapping the
  strip is **day-aware** (the tapped column resolves to its date via the
  spanning bars' `locationX ÷ cellSize` arithmetic): a **trip day's** segment
  opens **that trip's view on that day** — `TripDetail` with the tapped column's
  date and `focus: 'weather'`, landing on the day itinerary held at the top of
  its scroll, where the destination's forecast card for exactly that day sits
  (trips.md, "Day itinerary") — while any other day's segment opens the Weather
  screen as always.
...
- **The Weather screen's location source** is chosen on the screen itself via
  ...
  - **Another location**: the row closes the sheet and **pushes a full search
    screen** (`WeatherLocationSearch`, title "Another Location") — the same
    architecture as the event form's Location screen, and deliberately NOT a
    field inside the sheet: a bottom sheet rides on top of the keyboard, so a
    field's suggestions hanging below it render behind the keys (the shipped
    bug, twice — as an inline dropdown and as an in-sheet search step; both
    were unusable on an SE-sized screen). On the pushed screen the auto-focused
    **Google-Places city field** (`PlacesAutocomplete type="city" expand` — the
    same search as trip destinations) hangs from the header, where the keyboard
    can never reach it, and the matches fill the rest of the screen, ending at
    the keyboard's top edge (KAV inset offset by the header height). The
    results region is always present — before a query it says what to type;
    a query that matches nothing says so. **Selecting a suggestion is the
    confirmation**: it persists the source (`setWeatherSource`) and pops
    straight back to Weather, which re-reads the source on focus
    (`useFocusEffect`); free text alone is never accepted, so an unrecognized
    place can't be saved, and backing out changes nothing. The chosen place
    shows as the row's subtitle, and its weather is geocoded client-direct
...
- **Thunderstorm is the same composite, three-tone**: the solid white cloud
  ...
  and a **gold** bolt (blue lower band + a gold centre strip re-coloring just
...
- **Passive weather surfaces** (the calendar day view's **hourly weather
  ...
  follow the same chosen source, with one hard rule: they **never prompt** —
  the location permission ask belongs to the Weather screen. Live is used only
  ...
  the day-view glance once they've granted the permission on the Weather
  screen. The calendar surfaces (rail, glance, month strip) read
  `loadCalendarForecast` — the trip-aware layer below — while the assistant's
  context stays on the plain `loadPassiveForecast` (its weather grounds
  home-area suggestions).
- **Trip days show the destination's weather.** A forecast day that falls
  inside a trip's date range (trips add-on owned, trip has a
  destination and both dates — trip status no longer exists, so the dates are
  the whole gate) is swapped for the **destination's** forecast
  for that same date — you won't be home that day — while every other day
  keeps the chosen source. `loadCalendarForecast`
  (`lib/weatherSource.ts`) wraps `loadPassiveForecast` and overlays the
  destination days via the pure `applyTripForecast` (first matching trip wins
  a date; a destination day that's missing — fetch failed, date past its
  window — leaves the home day in place). The swapped days carry a `place`
  tag (the destination's leading segment, "Tokyo" out of "Tokyo, Japan") and
  the owning trip's `tripId` (what the strip's day-aware tap navigates to),
  and the surfaces render the place as an **airplane marker**: the month strip's
  segment leads with a small airplane, the List header's glance shows
  airplane + place before the icon and temps; the hourly rail swaps its marks
  wholesale (destination hours, no room for a glyph). The override is
  **derived, never written** — the Weather screen's persisted source pref is
  untouched, so leaving the trip window simply restores the chosen source.
  Trip resolution **fails open**: a locked add-on, an undecryptable list, or
  a failed destination fetch just renders home weather. Destinations resolve
  client-direct against keyless open-meteo (`geocodePlace`), never through
  our server. The Weather screen itself already covers the tap-through: while
  a trip is underway it appends the "On your trip" destination card
  under the home forecast.
...
- **Travel-aware weather.** When a trip's date range spans today, the
...
- **Holidays** and **occasions** (from Contacts) surface as read-only events (see
...
- The **Occasions** calendar (formerly "Birthdays") derives read-only,
  annually-recurring events from Contacts. Two sources per contact:
  - the dedicated `Contact.birthday` field → a `birthday` occasion; and
  - each `Contact.dates[]` entry → an occasion whose **kind comes from the
  ...
  (`{ id, kind, name, date, contactId, label, year? }`); consumers title/icon
  each kind via `mobile/src/lib/occasions.ts`. Add/edit occasion dates on the
  contact's card (Contacts); the internal calendar/add-on id stays `birthdays`.
- **Rendering.** On the month grid, occasions appear as **kind icons** (cake /
  ...
  contact's edit form, and passes a `focus` param so the list **scrolls that
  occasion to the top and outlines it** (bolder border + faint accent wash). The
  focus is matched by `occasionFocusKey` (contact + kind + month/day + label). (The
  contact's card is still where dates are edited, reached from the Occasions list.)
- **Per-contact inclusion.** A contact may be **excluded** from the Occasions
  calendar via `Contact.occasionsHidden` (sealed; default shown) — a
  ...
  toggling the switch back on from the contact's card. Tapping any (shown)
  occasion row opens the contact **scrolled to the Dates section**
  (`ContactForm` `focus: 'dates'`) to edit its dates.
- **Occasions list — a today-anchored timeline.** The list is not a flat
  ...
  `lib/occasions.collectOccasions(contacts, now?)` anchors each occasion to the
  ...
  **positive** offset (rolling to next year once this year's date is >7 days
  ...
  - a **"Today · `<Mon D>`"** marker row (accent rule + label) anchoring the
    boundary — shown whenever any occasions exist. **Today's occasions
    (`offset === 0`) render directly beneath the marker**, above the "Coming up"
    header (they're happening, not coming up), with the accent outline and the
    "Today" when-label. A today row whose card **already went out** shows the
    same **"Sent"** pill *in place of* the e-card envelope — the card sends once
    then deactivates, so the row must not invite scheduling a second card for a
    date that was just observed (a still-active card, i.e. one the user
    re-scheduled, keeps the edit envelope).
  - **Coming up** (`0 < offset ≤ COMING_UP_DAYS` (60)) — the highlighted
    ...
    (schedule / edit / active-`email-check`) lives. When nothing falls in the
    window an inline "Nothing in the next 60 days." note shows.
  - **Sent-pill matching is recency-guarded**: sent cards persist server-side
    forever and match rows by contact + kind + month/day, so only a card with
    `sentAt` within the past window (≤ `PAST_WINDOW_DAYS`+1 days ago) counts —
    last year's sent card never labels this year's occurrence as "Sent".
  ...
  (not a separate thing) and says "contacts", not "Contacts".
...
- **Scheduled e-cards.** From an occasion the user can schedule an e-card:
  ...
  **multi-select in one library visit** (`pickImages`, `selectionLimit` capped
  ...
  roster contact) — not the whole roster. To add another recipient, an **"Add a
  related contact"** row opens the occasion **contact itself** (`ContactForm`,
  ...
  default and "2:00 AM" isn't mistaken for "2:00 PM". **Scheduling on the
  occasion day itself** (the occasion is *today*): a send hour whose hourly tick
  has already passed by the device clock is **not offerable or submittable** —
  the picker filters to hours `> now.getHours()` (the current hour's tick has
  fired, so the next tick is the earliest reachable), the 9 AM default and an
  edit-mode seed **clamp forward** to the first reachable hour, and a save-time
  backstop alert ("That time has passed") catches the chosen hour passing while
  the form sat open. A reachable hour genuinely sends *at* that hour: the send
  pass cron-fires at minute 0 of every hour and delivers when
  `localHour(authorTz) ≥ sendHour` on the occasion's month/day. **Past 11 PM**
  no tick remains today, so the form keeps the full hour list, the hint's year
  advances, and an appended hint line says the card waits for the next
  occurrence. Enforcement is client-side (device clock ≈ the author's account
  timezone the scheduler resolves); the scheduler's at-or-after catch-up
  remains as the safety net for server downtime, not a scheduling affordance.
  The form's centerpiece is
...
- **A WidgetKit "Today" widget** (small / medium / large;
  `mobile/targets/widget/`, bundle id `app.householdcalendar.mobile.widget`)
  shows the day exactly as the in-app day view routes it: all-day rows first
  (events, trips, holidays, occasions, tasks, chores, meals, the shopping
  day), then timed events sorted by start — the SAME
  `itemsForDate → visibleDayItems → normalizeDay` pipeline, with the user's
  effective calendar colors, calendar-visibility toggles, custom-calendar
  access filtering and add-on locks all applied at build time, and cancelled
  occurrences rendered struck. Timed events that have ended drop off as the
  day progresses; tapping the widget opens the app.
- **Presentation is Apple-Calendar-widget style.** Accent weekday eyebrow over
  a large day number (full size on EVERY family, small included); timed
  events as two-line calendar-color-tinted chips (color bar + title +
  locale-aware compact time range, e.g. "4–5 PM", all in the calendar
  color); all-day items wear the SAME tinted-chip dress on ONE line — title
  leading, kind trailing ("All day", "Chore", "Holiday", "Meal", …) — one
  visual language across the widget (solid capsule pills and two-line all-day
  chips were tried and rejected on device); struck rows strikethrough +
  dimmed. Medium and large fill spare space with **upcoming covered days**
  under "TOMORROW" / "SATURDAY, AUG 15" eyebrow headers.
- **Layout law: plan against a line budget, never clip.** Content is planned
  per family against explicit line budgets (timed chip = 2 lines; all-day
  chip, eyebrow, and the "+N more" tail = 1), so the widget never emits a
  chip it can't fully show — no half-chips at the bottom edge (the device
  repro that forced this) — and the date header is laid out first and never
  moves or shrinks because of content below it. **Today outranks the
  future**: on medium, today's overflow spills from the left column into the
  right, and upcoming days get only what's left; on large, upcoming days fill
  only the space after today's full list (which carries the "+N more" tail
  when it alone overflows the budget).
- **The widget never decrypts and never fetches.** The extension holds no keys
  and has no network path to the API. The app writes a **14-day snapshot**
  (`lib/widgetSnapshot.ts`, `WINDOW_DAYS`) of expanded occurrences into the
  App Group container (`group.app.householdcalendar.mobile`) through the
  `modules/calen-widget` native bridge; the widget's timeline provider only
  reads that file. Writes happen on sign-in/mount, on app foreground, ~1.5 s
  after any `['calendar']` invalidation (`hooks/useWidgetSnapshot.ts` —
  mounted independently of the reminders toggle: the widget is not a
  notification), and from both background lanes (the silent-push record sync
  task and the periodic background-fetch slot). Every write ends by reloading
  the WidgetKit timelines.
- **Freshness contract: one app open a week is enough.** The widget's timeline
  emits an entry at each local midnight through the snapshot window (plus one
  at each of today's event ends), so the display self-advances for up to 14
  days with no app run — a weekly open keeps ≥ 7 days of margin, and the
  background lanes usually keep it fresher than that.
- **Degraded states are explicit and distinct.** A day inside the window with
  zero items renders "No events today" (a calm, correct fact) — but a day
  whose events have all **ended** renders "No more events today" instead
  (finished events drop off during the day, and "No events today" at 5pm
  after a morning of events reads as data loss, not done-for-the-day; this is
  Apple's own wording). A day **past the window's end** renders "Open Calen
  to update" — stale coverage must never masquerade as a free day. No
  snapshot at all (fresh install, post-sign-out) renders an "Open Calen"
  placeholder. A locked vault skips the write and leaves the last snapshot
  standing (it is the same signed-in user's data).
- **Lifecycle hygiene:** sign-out and household-change teardown delete the
  snapshot and reload timelines (`clearWidgetData`), in the same breath as
  the replica wipe.
- **Adoption surfaces (the widget must be found to be used — iOS offers no
  install API):** the **WidgetPromo** modal
  (`screens/calendar/WidgetPromoScreen.tsx`, root route `WidgetPromo`,
  `modalTask` chrome) pitches the widget with a mock of the **small family
  rendered from the user's own data** — `computeWidgetSnapshot()` (the
  compute half of the snapshot writer, exported precisely so the promo and
  the App Group file can never disagree), showing the first covered day that
  holds anything; an empty fortnight or a locked vault falls back to the
  widget's own sample day, labelled as a preview. Below it: a data-stays-on-
  device line and the three manual add steps (touch-and-hold → Edit / **+**
  → Add Widget → search "Calen" — worded to cover both the iOS 18 "Edit"
  flow and the older "+"). The screen asks WidgetKit whether a Calen widget
  is already installed (`installedWidgetCount` on the native bridge) and, if
  so, leads with a checkmarked "already on your Home Screen" card and
  retitles the steps "Add another size". Entry points: the one-time **widget
  nudge** (cadence normative in
  [notifications.md](notifications.md) "Widget nudge") and the durable
  **Profile promo card** (iOS only) — the widget is something the user does
  not have yet, not a setting they own, so its Profile surface is an ad, not
  a menu row: it sits **below every menu group** (under Help & support, above
  Sign out) as a full-width tappable card carrying a miniature of the small
  widget (today's weekday eyebrow + day number + color-tinted chips, drawn
  locally in the widget's visual language — the teaser is decorative; the
  user's OWN day is what the promo screen renders), the pitch, and an "Add
  the widget" chevron line. Tapping anywhere on it opens WidgetPromo.
...
- **Model:** `CalendarEvent` (`server/src/models/CalendarEvent.js`). Custom
  ...
  `mobile/src/lib/householdRsvp.ts`). Reconciling the two lanes when the parties
  join one household: `mobile/src/lib/invitationMerge.ts` +
  `GET /api/invitations/reconcile` / `POST /api/invitations/:id/merge`.
...
- **The widget snapshot is a deliberate device-local plaintext exception.**
  The App Group file (`widget-snapshot.json`) holds decrypted data for the
  next 14 days, outside the sealed replica but **never off the device** — the
  server is not involved in any direction. It is minimized to exactly what the
  widget draws: titles, clipped minutes-from-midnight times, colors, a
  struck flag — no record ids, descriptions, locations, or invitees. It is
  written with iOS file protection `completeUntilFirstUserAuthentication`
  (encrypted at rest until the first post-boot unlock, then readable by the
  widget even while the phone is re-locked — a lock-screen-adjacent surface
  must render there), and it is deleted on sign-out and household change.
- **One cache key, one fetcher — `['invitations']` is defined in
  `lib/eventInvitations`, nowhere else.** A D3 sealed invitation has no
  plaintext `event`, so the list has to be **fetched with its snapshots
  opened**; the inbox renders "Unlock to view this invitation" for any row that
  arrives unopened. Three surfaces read this key — the inbox, the badge count
  (`useInvitationsCount`, mounted on the calendar home AND Profile), and the
  event form's "who invited me" line — and React Query caches by key alone, so
  **whichever observer triggers a fetch decides what all three receive**. While
  only the inbox's copy decrypted, any badge refetch overwrote the cache with
  undecrypted rows and the inbox, opened seconds later, served them straight
  from cache: a padlock shown to someone whose vault was open, cleared only by
  pull-to-refresh (the one fetch the inbox's own observer drives). Reaching the
  inbox from Profile hit it almost every time, and a sign-out guaranteed it by
  clearing the cache for the badge to repopulate first. The key, its fetcher,
  and every `invalidateQueries` reference now come from that one module; a test
  scans the source and fails on any inlined `queryKey: ['invitations']`, because
  each reader is individually correct and the defect exists only in the
...
- The `['invitations']` fetcher opens sealed snapshots, passes plaintext rows
  through untouched, and keeps a row it couldn't open — plus the structural
  guard that no file inlines that query key (the defect lived in the
  relationship between three individually-correct readers, so a behavior test
  alone cannot catch it) —
  `mobile/src/lib/__tests__/eventInvitations.test.ts`.
- What a hidden calendar removes from a day (the collection → calendar-id
  mapping every surface shares, grocery marker included) —
  `mobile/src/lib/__tests__/visibleDayItems.test.ts`.
- Custom calendars: create/list visibility tiers (private, household-wide,
  member-specific, outsiders excluded), the calendar-record write split
  (full-access housemates edit basics; sharing/holiday/delete stay the
  creator's; view-only members, outside collaborators, and non-shared members
  refused), validation,
...
- Invitation merge (the parties later join one household): the work list is
  recipient-scoped and only lists invitations whose organizer is now a housemate,
  merging tombstones the duplicate copy and retires the row to `merged`, merging
  is idempotent, a merged row leaves both inboxes and the guest list, an orphaned
  copy is kept rather than tombstoned, and accept/leave/revoke 409 for the window
  before the pass runs — `invitationMerge.integration.test.js`. The device-side
  decision table (invite me / carry my answer / unlink my copy / defer), the
  sealed-work-before-retirement ordering, and per-row failure isolation —
  `mobile/src/lib/__tests__/invitationMerge.test.ts`.
...
- Alerts managed in place on the event view: the row reads the saved alert back
  (not a placeholder), the sheet offers the form's list including the
  departure-anchored rows on an event with a drive time, a pick writes straight
  to the event without ever opening the form, a departure pick stores the
  `leave` framing beside the computed minutes, clearing the first alert promotes
  the second, and an event with no alert still shows the picker with the second
  slot hidden — `mobile/src/screens/calendar/__tests__/EventDetailScreen.alerts.test.tsx`.
  That the write goes through the sealed re-seal lane — carrying the untouched
  event content, and dropping a cleared slot out of the sealed subset — is
  pinned beside the guest-list and mark-cancelled flips in
  `mobile/src/screens/calendar/__tests__/sealedEventFlips.test.ts`.
...
- The account-backed arrangement and the first-paint gate —
  `calendarPrefs.test.ts`: the account's colors/order/hidden/deleted/muted are
  adopted on the next sign-in, an account value stored as empty beats the cache,
  a device with no account copy seeds it, a recolor pushes the whole
