...
- **Invite from contacts (mobile):** the invite field on `HouseholdScreen`
  ...
  (`matchRoster`), also backing the **calendar outside-share field**
  (AddCalendarScreen, which *stages* instead of sending; details in
  [calendar](calendar.md)). Each caller keeps its own taken set and send/stage
  semantics.
