...
- **A successful import hands the form to the assistant.** Once a quick
  import (URL or photos) populates the form, the Quick import card gives way to
  the floating "Ask Calen" pill the edit/review modes also show, so the imported
  recipe can be refined ("make it vegan", "double the servings") before it is
  first saved. An import also **resets the conversation** — it replaced the whole
  recipe, so anything said before was about a different dish.
- **The form assistant is the shared `FormAssistChat` pill + sheet** (the same
  "Ask Calen" entry point every add/edit form shows, per mobile/CLAUDE.md and
  [ai-assistant.md](ai-assistant.md)). A plain edit shows the resting pill; a
  form **just populated by a quick import or an AI review** pulses it once
  (`attention`) — refining the import is the expected next step — rather than
  auto-opening the sheet over a screen the user has only just landed on, so the
  keyboard stays
