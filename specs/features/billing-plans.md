...
- **A locked viewer lands on `ViewerUnlock`, and it IS the shell.** When the
  ...
  is what makes this safe both ways: RootNavigator holds the splash until auth
  bootstrap (including its silent biometric/passkey attempt) settles, and once
  the user is back in, the navigator does not re-mount and yank them out.
