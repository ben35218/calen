...
- **The screen is written for someone who has never heard the word "key."** It
  ...
  3. **"Request access"** — re-keys ONCE and asks every owner. One action
     ...
     *for* ("Use the password you sign in with now. It becomes this phone's key…")
     ...
     their account is the "You" Contact the client auto-seeds at boot
     (`lib/selfContact`) — which the server's guard counts, telling someone who
...
- **The home is a calendar, with the agenda behind a toggle.** Someone
  ...
  - **Grid layer (`ViewerMonthGrid`, the default):** a read-only cousin of the
    ...
    1st marked with the abbreviated month name in the app primary sitting on
    top of the row's opening rule (the ordinary `colors.border` hairline drawn
    as each own-month cell's label-slot bottom border, over the month's own
    days only — calendar.md owns the rule) —
    including the
    ...
    **stays pinned there**, re-snapping on every offset change and content
    ...
    bars. The chips carry the unlocked grid's **tinted** styling exactly (a
    translucent wash of the calendar's color under title and time drawn in
    that color, contrast-corrected by `lib/color.tintedChip`; spans stay
    solid with white text — calendar.md owns the rule).
    A chip or bar opens `ViewerEvent`; a day opens that day's full list
  ...
  - **List layer (`ViewerAgendaList`):** the day-grouped upcoming-events agenda
    ...
    color, and the list reads as "what's coming up", not "whose". Days are grouped by the date the event
...
- **Chat is TOKEN-PRICED (the one variable action).** A chat turn's cost grows
  ...
  never debited.
  **`POST /form-assist` bills the same way**, and this is a correction: it meters
  under the `chat` action, so `meter()` has always skipped its flat debit — but
  unlike the chat routes it never called `recordChatCredits`, which meant every
  "Ask Calen" form fill since it shipped cost the user **nothing**. It now
  debits its turn from `usageBreakdown(resp.usage)` and returns `creditsUsed` on
  the JSON response (there is no stream). The route is non-streaming precisely so
  `services/aiUsage.js`'s SDK patch keeps recording the tokens it is billing —
  that patch skips `{ stream: true }` calls. Its transcript is capped at 8 turns
  server-side, and mirrored client-side, so the bill can't grow unbounded with
  the conversation; see [ai-assistant.md](ai-assistant.md). **Web search inside a chat is NOT a separate charge** — it
...
- Add-ons come in two classes, both acquired on the **"Add-ons" store screen**
  ...
  CTAs.
- **Discovery: the full-screen Discover modal** (`screens/plan/DiscoverScreen.tsx`,
  route `Discover`, `modalTask` presentation — header ✕ + swipe-down). What the
  **discovery nudge** presents from the device's third app open on a spaced,
  capped cadence (cadence/priority owned by
  [notifications.md](notifications.md) "Discovery nudge"): a hero, one
  `CardRow` per **unowned** add-on (accent `IconAvatar`, registry display name,
  a local one-line pitch, free ones badged "Free") routing at the Add-Ons store
  with that add-on focused, a ghost "See all add-ons", and a **"Brainstorm with
  Calen"** card routing at the assistant. The purchase/claim CTAs stay on the
  store screen (where Restore + legal links live — App Review 5.2.5); the modal
  only routes there, and it **closes itself before navigating** so the target's
  back gesture lands on the calendar, not a stale promotion. The render reads
  the **live** owned set (`useOwnedAddons`), so an owned add-on is never
  promoted even when the nudge decided off a stale mirror; with every add-on
  owned the add-on section disappears entirely and the modal is purely the
  brainstorm pitch (which the nudge shows at most once in that state).
...
- **No grandfathering, no defaults:** a household without an add-on (unbought
  ...
  deleted) and MUST reappear, with the user's prior visibility/color/order
