...
title: API reference
status: current
last-verified: 8f4f188+ (2026-09-23); pinned the **401-vs-403 convention** — 401 is reserved for a dead session (missing/expired token, revoked session row; the mobile client signs out on it), 403 for a failed credential input on a valid session; the wrong-current-password lanes on `PUT /auth/email`, `PUT /auth/password` and `DELETE /auth/account` moved 401→403 accordingly (behavior owned by features/auth-identity.md "Session persistence & restore"; pinned in authFlows.integration.test.js) (2026-09-23); 1b68f83+ (2026-09-06); documented `POST /form-assist`'s conversational contract (`messages` transcript alongside the legacy one-shot `prompt`; contract owned by features/ai-assistant.md) (2026-09-06); d3dea32+ (2026-08-22); `PUT/GET /settings`'s `calendarPrefs` gained `groupOrder` (the audience-section sequence), normalized and bounded on the same lane as its other id lists; contract owned by features/calendar.md (2026-08-22); 3cfa750+ (2026-08-15); documented `GET/PUT /recipe-schedule/session`'s version contract (sealed envelope + 409-with-current-version on stale `baseVersion`; legacy plaintext lane during the sealing transition) and `POST /recipes/from-url`'s SSRF-guard 400 — contracts owned by features/kitchen.md (2026-08-15); 3cfa750+ (2026-08-14); `/api/records/ws`'s change-stream lane made self-healing — 'close'/'end' now reconnect like 'error' (one restart per death), the 5s→60s backoff resets once the stream proves healthy (first change event or 30s alive), and death/recovery log loudly; a silent close previously killed cross-instance poke fanout until process restart (2026-08-14); 3cfa750+ (2026-08-13); `GET /records/sync` — fixed the cursor race that permanently skipped records: `serverTime` was stamped AFTER the query returned, so a write committing mid-pull (updatedAt < serverTime, absent from the response) fell forever behind the client's `$gt` cursor; `serverTime` is now captured before the query minus a ~5s overlap (`RECORDS_SYNC_OVERLAP_MS`), so such rows re-deliver on the next pull — clients reconcile idempotently by `_id` (2026-08-13); 1d42ed2+ (2026-08-12); `/api/invitations` gained the `reconcile` / `:id/merge` pair — retiring an event invitation once its two parties join one household (2026-08-12); ddaa21b+ (2026-08-09); documented the `/places/autocomplete` `type` contract and fixed the untyped lane — it filtered to `includedPrimaryTypes: ['establishment']`, so the "business or address" fields (event Location, trip-item location) could never match a street address; body-building extracted to the pure `autocompleteBody()` helper with unit coverage (2026-08-09); ddaa21b+ (2026-08-08); added the `/api/records/ws` record-change poke WebSocket (poke-and-pull: content-blind `{"type":"changed"}` frames on household record writes, session-auth'd upgrade, writer-session excluded; fed by `services/recordChanges.js` — write-route announcements + a Mongo change stream where a replica set exists) (2026-08-08); ddaa21b+ (2026-08-06); `/api/notifications` gained the stateless household event notify relay (`POST event-request`/`event-response` — client-chosen push strings, membership-validated, rate-limited, nothing stored; behavior owned by features/notifications.md) (2026-08-06); c2d18c0+ (2026-08-04); `PUT/GET /settings` accepts/echoes `calendarPrefs` — the user's calendar arrangement (colors / order / hidden / deleted built-ins / muted alerts), merged field-by-field so a partial payload can't blank the rest, validated (hex colors, string ids, 500-entry cap) with 400 on malformed input, `null` = never configured (2026-08-04); `GET /billing/status` gained the free-viewer-mode `viewer` counts, and `/records` now authorizes calendar-lane writes from the plaintext `scope` (403 for `view` collaborators / foreign-scope claims; trips exempt) (2026-07-31); added `/api/ecards` (plaintext occasion e-cards) (2026-07-28); added e-card photo endpoints (upload/serve/delete) (2026-07-28); `PUT/GET /settings` accepts/echoes personal `dayAlertTime` (HH:mm, empty=reset to 9am default; validated) (2026-07-29); `PUT/GET /settings` accepts/echoes `homeCity` — a coarse plaintext household home-area label (city + region/country) derived client-side from the home address (or hand-set) that grounds the calendar assistant's local suggestions without ever exposing the street address (2026-07-30); **the two calendar-level alert configs (Occasions + holidays) are now ACCOUNT settings** — `User.occasionAlerts` / `User.holidayAlerts`, carried on `GET`/`PUT /settings`, with the AsyncStorage keys demoted to a cache: that cache is account state wiped at sign-out, so holiday alerts a user set read back fine all session and were silently off again at the next sign-in; edits now write both, load adopts the account's config (rescheduling the window when it differs), an account with no config is seeded from a device holding a non-default one, and `offsets: []` stays a real "off" distinct from an unconfigured `null` (c2d18c0+, 2026-08-04); documented the recipe photo pair (`POST /recipes/photo` upload — no AI, no meter — and `PUT /recipes/:id/photo`, the claim that binds a file to a saved recipe because the sealed `imageUrl` is unreadable server-side) and the record-delete file cascade now reaping recipe photos alongside event attachments (2026-08-11)
code:
...
- **401 vs 403:** 401 means THIS SESSION IS DEAD (missing / expired token,
  revoked session row) — the mobile client signs the device out on it (scoped;
  see [auth-identity](../features/auth-identity.md) "Session persistence &
  restore"). A failed credential INPUT on a valid session — wrong
  `currentPassword` on `PUT /auth/email` / `PUT /auth/password`, wrong
  `password` on `DELETE /auth/account` — answers **403**. New session-authed
  routes MUST follow this split; a 401 for anything but session death signs
  real devices out.
...
| Method | Path | Purpose |
|---|---|---|
| GET | `/api/records/sync` | Incremental **last-writer-wins** pull: every record in the caller's scope changed after `?since=<cursor>`, including tombstones for deletes. The response's `serverTime` (which the client commits as its next `since`) is captured **before** the query runs, backdated by an overlap epsilon (~5s, `RECORDS_SYNC_OVERLAP_MS`) covering write-stamp→commit latency and cross-instance clock skew — the cursor must always predate the query snapshot, or a write committing mid-pull (stamped before the snapshot but invisible to it) falls permanently behind the `$gt` cursor and is skipped until its next edit. Consequence: rows updated within the overlap window (tombstones included — they ride the same cursor) may be **re-delivered** on consecutive pulls; clients MUST reconcile idempotently by `_id` (the replica's LWW upsert does). |
...
- **Calendar:** `/api/calendars` (custom calendars + per-calendar key envelopes +
  ...
  (event invitations incl. public `ics` + `lookup`, plus the `reconcile` /
  `:id/merge` pair that retires an invitation once its two parties join one
  household). See [features/calendar.md](../features/calendar.md).
...
- **Kitchen:** `/api/recipes` (incl. `suggest-recipes`), `/api/recipe-schedule`,
  ...
  use. Photo **bytes are plaintext**, as recipe images have always been.
  `GET/PUT /api/recipe-schedule/session` carries the sealed shopping-session
  envelope (`enc`/`keyVersion`) plus an optimistic-concurrency `version`: a PUT
  with a stale `baseVersion` writes nothing and answers **409 + the current
  `version`** (the client re-fetches, merges on-device, retries); a PUT with no
  `baseVersion` is the legacy plaintext lane, accepted LWW during the sealing
  transition. `POST /api/recipes/from-url` (and the og:image fetch behind it)
  runs every outbound fetch through the SSRF guard — non-http(s), private,
  loopback, link-local, and metadata addresses answer **400**, re-checked on
  every redirect hop. Contracts owned by
...
- **AI:** `/api/calendar/chat`, `/api/maintenance/chat`,
  ...
  - `POST /form-assist` takes the form's field schema + current values plus
    EITHER a `messages` transcript (the Ask Calen sheet — the server answers
    conversationally and may return a clarifying question with no patch) or the
    legacy one-shot `prompt` (older app builds — the fill stays mandatory).
    Answers `{ patch, reply, note, creditsUsed }`; `note` aliases `reply` for
    those older builds. Token-priced under the `chat` action. All AI routes
...
- **Misc:** `/api/weather`, `/api/notifications`, `/api/settings`,
  ...
  - `PUT/GET /settings` also carry `calendarPrefs` — how the user arranged their
    ...
    `{ colors: { [calendarId]: "#RRGGBB" }, order: string[],
    groupOrder: ("household"|"justMe"|"shared")[], hidden: string[],
    deletedDefaults: string[], alertsOff: string[] }` — `groupOrder` is the
    sequence the audience sections list in — every field optional and
    ...
    storing it (non-hex colors, non-string ids, over 500 entries). `null` means
