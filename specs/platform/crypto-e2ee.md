...
### The device key cache: two tiers, both owner-bound

The relaunch cache (`cacheKeyPairToDevice` → `lib/deviceKey`, the serialized
identity keypair) is **two keychain copies of the same owner-stamped blob**,
and which one a cold start may use is decided by the App Lock preference:

- **Silent tier** (`hc_device_key_silent`, no `requireAuthentication`): exists
  **only while App Lock is "Never"** — the default — and lets
  `unlockFromDeviceCache` restore keys with **zero prompts**, so a cold start
  opens straight onto decryptable data. This is deliberate, not a weakening:
  the record replica beside it already stores decrypted rows in plain
  AsyncStorage/SQLite, so both are phone-passcode-level at rest (and
  `WHEN_UNLOCKED_THIS_DEVICE_ONLY`, never migrating off the phone). Gating the
  key behind biometrics while the content sat ungated bought a prompt per
  cold start for no confidentiality — the original design assumed a ciphertext
  replica that never shipped. Signal's model, matched by the pref's default.
- **Biometric tier** (`hc_device_key`, `requireAuthentication: true`): the
  Face ID / Touch ID gated copy, the only tier once ANY App Lock window is
  chosen. `applyAppLockPolicy(minutes)` (called by the pref UI) deletes the
  silent copy the moment a window is picked — effective at the NEXT cold
  start, not the next unlock — and re-arms it in place when the pref returns
  to Never. Every unlock path ends in `cacheKeyPairToDevice`, which rewrites
  both tiers per the pref; a biometric-tier unlock also self-arms a missing
  silent copy (installs predating the tier). The unlock path re-checks the
  pref before trusting a silent blob, so a stale copy surviving a crash
  between pref-write and delete is ignored and removed.

Both blobs **stamp the owning account's user id**, and `unlockFromDeviceCache`
refuses — and deletes — a blob stamped for a different account than the one
unlocking (a legacy unstamped biometric blob is accepted once and re-written
with its owner; a foreign blob on either tier drops both, since they are
written together). The stamp exists because the cache **outlives a
session-expiry sign-out**: store/auth keeps it on the 401 teardown so a weekly
token expiry costs at most one glance — and, under "Never", nothing at all
(normative in features/auth-identity.md "Session persistence & restore").
Without the binding, account B signing in on a device whose cache still held
account A's keypair could have A's identity key restored into B's session. A
deliberate sign-out still wipes both tiers (`forgetDeviceKey` /
`clearDeviceKey`, which always forgets the silent copy too), as does a re-key.
Pinned in e2ee.test.ts "device-cache owner binding" + "silent device-key tier".
