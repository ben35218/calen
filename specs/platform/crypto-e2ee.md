...
### The biometric device cache is owner-bound

The Face-ID relaunch cache (`cacheKeyPairToDevice` → `lib/deviceKey`, the
serialized identity keypair behind the OS biometric gate) now **stamps the
owning account's user id into the blob**, and `unlockFromDeviceCache` refuses —
and deletes — a blob stamped for a different account than the one unlocking
(a legacy unstamped blob is accepted once and re-written with its owner).
The stamp exists because the cache **outlives a session-expiry sign-out**:
store/auth keeps it on the 401 teardown so a weekly token expiry costs one
Face ID glance instead of a typed password (normative in
features/auth-identity.md "Session persistence & restore"). Without the
binding, account B signing in on a device whose cache still held account A's
keypair could have A's identity key restored into B's session. A deliberate
sign-out still wipes the cache (`forgetDeviceKey`). Pinned in e2ee.test.ts
"device-cache owner binding".
