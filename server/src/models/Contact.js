...
// names extend it with an optional link to another Contact. Under E2EE these ride
// inside the sealed `enc` blob (client mirror: lib/contactFields); the plaintext
...
const contactSchema = new mongoose.Schema({
  ...
  // When set, this Contact is the self-record for that household member's User
  ...
  // mirror: lib/contactFields + encSubsets CONTACT_ENC). Optional — legacy records
  ...
  // The device address-book id this Contact was imported from, when applicable.
...
// Ensure the given User has a linked self-record in the Contacts roster, creating
// one from the account's profile fields on first call. Idempotent and cheap
// (indexed findOne when the record already exists). Returns the self Contact.
contactSchema.statics.ensureSelf = async function (user) {
  ...
  if (!self) {
    ...
    // Contact after first unlock — so the server must not create a plaintext one.
    ...
    const hh = user.householdId
      ...
      : null;
    if (hh?.e2eeActive) return null; // client seeds the encrypted self-Contact
  ...
  if (!user.contactId || String(user.contactId) !== String(self._id)) {
    await mongoose.model('User').updateOne({ _id: user._id }, { $set: { contactId: self._id } });
  }
  return self;
};
