...
const RULES = [
  ...
  { re: /^(server\/src\/routes\/notifications\.js|server\/src\/jobs\/scheduler\.js|server\/src\/services\/(push|notify)\.js|mobile\/src\/(lib\/(notifications|push|backgroundRefresh|securityNudges|discoverNudge|widgetNudge)\.ts|hooks\/(useReminderScheduler|usePushNotifications|useInviteAlerts|useSecurityNudges|useDiscoverNudge|useWidgetNudge)\.ts|screens\/profile\/RemindersScreen\.tsx))/, specs: ['specs/features/notifications.md'],
    tests: /^(server\/src\/test\/notifications\.|server\/src\/jobs\/scheduler\.test\.js$|mobile\/src\/lib\/__tests__\/(notifications|rescheduleReminders|securityNudges|discoverNudge|widgetNudge)\.)/ },
